...
export class LandingArea extends Area
{
    ...
    setLetters()
    {
        ...
        // Sembunyikan huruf lama
        for(const reference of references)
        {
            reference.visible = false
        }

        // Buat XORA AGV dengan physics
        this._createXoraLetters(references)
    }

    _createXoraLetters(oldRefs)
    {
        // Hitung posisi dari huruf lama
        let cx = 0, cy = 0, cz = 0
        ...
        const charD = 0.4
        const gap = 0.15
        const startX = cx - ((text.length - 1) * (charW + gap)) / 2
        ...
        for(let i = 0; i < text.length; i++)
        {
            ...
            // Box mesh
            const geo = new THREE.BoxGeometry(charW, charH, charD)
            ...
            const posX = startX + i * (charW + gap)
            mesh.position.set(posX, cy + charH * 0.5, cz)
            mesh.castShadow = true
            mesh.receiveShadow = true
            this.game.scene.add(mesh)

            // Rapier physics — static rigid body + box collider
            const rigidBodyDesc = RAPIER.RigidBodyDesc.fixed()
                .setTranslation(posX, cy + charH * 0.5, cz)
            const rigidBody = this.game.physics.world.createRigidBody(rigidBodyDesc)

            const colliderDesc = RAPIER.ColliderDesc.cuboid(charW / 2, charH / 2, charD / 2)
            colliderDesc.setActiveEvents(RAPIER.ActiveEvents.CONTACT_FORCE_EVENTS)
            colliderDesc.setContactForceEventThreshold(5)
            const collider = this.game.physics.world.createCollider(colliderDesc, rigidBody)

            // Collision sound
            rigidBody.onCollision = (force, position) =>
            {
        ...
        // Point light glow
        const light = new THREE.PointLight(0x00d4ff, 2, 8)
        light.position.set(cx, cy + 3, cz + 2)
